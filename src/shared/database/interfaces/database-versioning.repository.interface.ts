...
export interface DatabaseVersioningInterfaceRepository<
...
> {
  ...
  findOneByVersion(id: string | number, version: number): Promise<T | null>;
  findAllVersions(
    id: string | number,
  ...
  saveNewVersion(data: DeepPartial<T>): Promise<T>;
  saveMany(data: DeepPartial<T>[]): Promise<T[]>;

  upsert(data: Partial<T>): Promise<InsertResult>;
  upsertMany(data: Partial<T>[]): Promise<InsertResult[]>;
